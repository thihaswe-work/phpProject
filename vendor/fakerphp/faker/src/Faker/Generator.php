...
class Generator
{
    ...
    {
        $this->container =
            $container ?: Container\ContainerBuilder::getDefault();
    }
    ...
    public function ext(string $id): Extension\Extension
    {
        if (!$this->container->has($id)) {
            throw new Extension\ExtensionNotFound(
                sprintf('No Faker extension with id "%s" was loaded.', $id)
            );
        }
    ...
    public function optional(float $weight = 0.5, $default = null)
    {
        if ($weight > 1) {
            trigger_deprecation(
                'fakerphp/faker',
                '1.16',
                'First argument ($weight) to method "optional()" must be between 0 and 1. You passed %f, we assume you meant %f.',
                $weight,
                $weight / 100
            );
            $weight = $weight / 100;
    ...
    {
        return $this->ext(Extension\NumberExtension::class)->numberBetween(
            (int) $int1,
            (int) $int2
        );
    }
    ...
    {
        return $this->ext(Extension\NumberExtension::class)->randomDigitNot(
            (int) $except
        );
    }
    ...
    {
        return $this->ext(
            Extension\NumberExtension::class
        )->randomDigitNotZero();
    }
    ...
    public function semver(
        ...
        return $this->ext(Extension\VersionExtension::class)->semver(
            $preRelease,
            $build
        );
    }
    ...
    protected function callFormatWithMatches($matches)
    {
        trigger_deprecation(
            'fakerphp/faker',
            '1.14',
            'Protected method "callFormatWithMatches()" is deprecated and will be removed.'
        );
    ...
    public function __get($attribute)
    {
        trigger_deprecation(
            'fakerphp/faker',
            '1.14',
            'Accessing property "%s" is deprecated, use "%s()" instead.',
            $attribute,
            $attribute
        );
