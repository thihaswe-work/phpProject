...
class File extends Base
{
    ...
    {
        $random_extension = static::randomElement(
            array_values(static::$mimeTypes)
        );

        return is_array($random_extension)
            ? static::randomElement($random_extension)
            : $random_extension;
    }
    ...
    public static function file(
        ...
        if (empty($files)) {
            throw new \InvalidArgumentException(
                sprintf('Source directory %s is empty.', $sourceDirectory)
            );
        }
        ...
        $sourceFullPath =
            $sourceDirectory .
            DIRECTORY_SEPARATOR .
            static::randomElement($files);

        $destinationFile =
            Uuid::uuid() . '.' . pathinfo($sourceFullPath, PATHINFO_EXTENSION);
        $destinationFullPath =
            $targetDirectory . DIRECTORY_SEPARATOR . $destinationFile;
