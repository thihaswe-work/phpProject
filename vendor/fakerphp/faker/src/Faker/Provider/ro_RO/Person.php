...
class Person extends \Faker\Provider\Person
{
    ...
    public function cnp(
        ...
        if (null === $county) {
            $countyCode = static::randomElement(
                array_values(static::$cnpCountyCodes)
            );
        } elseif (!array_key_exists($county, static::$cnpCountyCodes)) {
            throw new \InvalidArgumentException(
                "Invalid county code '{$county}' received"
            );
        } else {
        ...
        $cnp =
            (string) $this->getGenderDigit($date, $gender, $isResident) .
            $date->format('ymd') .
            $countyCode .
            static::numerify('##%');
    ...
    protected function getDateOfBirth($dateOfBirth)
    {
        ...
        $baseDate = \Faker\Provider\DateTime::dateTimeBetween(
            "first day of January {$dateOfBirthParts[0]}",
            "last day of December {$dateOfBirthParts[0]}"
        );
        ...
        if ($dateOfBirthParts[0] < 1800 || $dateOfBirthParts[0] > 2099) {
            throw new \InvalidArgumentException(
                "Invalid date of birth - year must be between 1800 and 2099, '{$dateOfBirthParts[0]}' received"
            );
        }
        ...
        if ($date->format('Y-m-d') !== $dateOfBirthFinal) {
            throw new \InvalidArgumentException(
                "Invalid date of birth - '{$date->format(
                    'Y-m-d'
                )}' generated based on '{$dateOfBirth}' received"
            );
        }
     ...
     * @return int
     */
    protected static function getGenderDigit(
    ...
    protected function getChecksumDigit($value)
    {
        ...
        foreach (range(0, 11) as $digit) {
            $checksum +=
                (int) substr($value, $digit, 1) *
                (int) substr($checkNumber, $digit, 1);
        }
