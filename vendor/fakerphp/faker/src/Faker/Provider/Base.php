...
class Base
{
    ...
    public static function shuffle($arg = '')
    {
        ...
        throw new \InvalidArgumentException(
            'shuffle() only supports strings or arrays'
        );
    }
    ...
    private static function replaceWildcard(
        ...
        for (
            $i = $pos, $last = strrpos($string, $wildcard, $pos) + 1;
            $i < $last;
            ++$i
        ) {
    ...
    public static function numerify($string = '###')
    {
        ...
        if (($pos = strpos($string, '#')) !== false) {
            for (
                $i = $pos, $last = strrpos($string, '#', $pos) + 1;
                $i < $last;
                ++$i
            ) {
                if ($string[$i] === '#') {
    ...
    public static function regexify($regex = '')
    {
        ...
        $regex = preg_replace(
            '/(?<!\\\)\*/',
            '{0,' . static::randomDigitNotNull() . '}',
            $regex
        );
        $regex = preg_replace(
            '/(?<!\\\)\+/',
            '{1,' . static::randomDigitNotNull() . '}',
            $regex
        );
        ...
        $regex = preg_replace_callback(
            ...
            static function ($matches) {
                return str_repeat(
                    $matches[1],
                    Base::randomElement(range($matches[2], $matches[3]))
                );
            },
            $regex
        );
