...
abstract class Text extends Base
{
    ...
    {
        return $this->realTextBetween(
            (int) round($maxNbChars * 0.8),
            $maxNbChars,
            $indexSize
        );
    }
     ...
     * @return string
     */
    public function realTextBetween(
        $minNbChars = 160,
        $maxNbChars = 200,
        $indexSize = 2
    ) {
        if ($minNbChars < 1) {
            throw new \InvalidArgumentException(
                'minNbChars must be at least 1'
            );
        }

        if ($maxNbChars < 10) {
            throw new \InvalidArgumentException(
                'maxNbChars must be at least 10'
            );
        }
        ...
        if ($minNbChars >= $maxNbChars) {
            throw new \InvalidArgumentException(
                'minNbChars must be smaller than maxNbChars'
            );
        }
        ...
        do {
            ...
            if ($iterations >= 100) {
                throw new \OverflowException(
                    sprintf(
                        'Maximum retries of %d reached without finding a valid real text',
                        $iterations
                    )
                );
            }
    ...
    protected function getExplodedText()
    {
        if ($this->explodedText === null) {
            $this->explodedText = static::explode(
                preg_replace('/\s+/u', ' ', static::$baseText)
            );
        }
