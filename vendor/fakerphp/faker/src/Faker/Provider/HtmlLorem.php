...
class HtmlLorem extends Base
{
    ...
    public function randomHtml($maxDepth = 4, $maxWidth = 4)
    {
        if (!class_exists(\DOMDocument::class, false)) {
            throw new \RuntimeException(
                'ext-dom is required to use randomHtml.'
            );
        }
    ...
    private function addRandomAttribute(\DOMElement $node)
    {
        ...
        switch ($rand) {
            ...
            case 2:
                $node->setAttribute(
                    'id',
                    (string) $this->idGenerator->randomNumber(5)
                );
    ...
    private function addRandomP(\DOMElement $element, $maxLength = 10)
    {
        ...
        $node->textContent = $this->generator->sentence(
            self::numberBetween(1, $maxLength)
        );
    ...
    private function addRandomText(\DOMElement $element, $maxLength = 10)
    {
        $text = $element->ownerDocument->createTextNode(
            $this->generator->sentence(self::numberBetween(1, $maxLength))
        );
    ...
    private function addLoginForm(\DOMElement $element)
    {
        ...
        $passwordInput = $element->ownerDocument->createElement(
            static::INPUT_TAG
        );
        $passwordInput->setAttribute('type', 'password');
        $passwordInput->setAttribute('id', 'password');

        $passwordLabel = $element->ownerDocument->createElement(
            static::LABEL_TAG
        );
    ...
    private function addRandomTable(
        ...
        for ($i = 0; $i < $cols; ++$i) {
            ...
            $th->textContent = $this->generator->sentence(
                self::numberBetween(1, $maxTitle)
            );
            $tr->appendChild($th);
        }
    ...
    private function addRandomUL(
        ...
        for ($i = 0; $i < $num; ++$i) {
            ...
            $li->textContent = $this->generator->sentence(
                self::numberBetween(1, $maxLength)
            );
            $ul->appendChild($li);
        }
