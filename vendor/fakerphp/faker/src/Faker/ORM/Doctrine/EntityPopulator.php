...
class EntityPopulator
{
    ...
    public function guessColumnFormatters(\Faker\Generator $generator)
    {
        ...
        foreach ($this->class->getFieldNames() as $fieldName) {
            if (
                $this->class->isIdentifier($fieldName) ||
                !$this->class->hasField($fieldName)
            ) {
                continue;
            }
        ...
        foreach ($this->class->getAssociationNames() as $assocName) {
            ...
            if ($this->class instanceof \Doctrine\ORM\Mapping\ClassMetadata) {
                ...
                foreach ($mappings as $mapping) {
                    if ($mapping['targetDocument'] == $relatedClass) {
                        if (
                            $mapping['type'] ==
                                \Doctrine\ODM\MongoDB\Mapping\ClassMetadata::ONE &&
                            $mapping['association'] ==
                                \Doctrine\ODM\MongoDB\Mapping\ClassMetadata::REFERENCE_ONE
                        ) {
                            $unique = true;
            ...
            $index = 0;
            $formatters[$assocName] = static function ($inserted) use (
                $relatedClass,
                &$index,
                $unique,
                $optional,
                ...
                if (isset($inserted[$relatedClass])) {
                    if ($unique) {
                        ...
                        if (
                            isset($inserted[$relatedClass][$index]) ||
                            !$optional
                        ) {
