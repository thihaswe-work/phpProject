...
class Populator
{
    ...
    public function addEntity(
        ...
        if (null === $mapper) {
            throw new \InvalidArgumentException(
                'No mapper can be found for entity ' . $entityName
            );
        }
        ...
        $entity->setColumnFormatters(
            $entity->guessColumnFormatters($this->generator)
        );
    ...
    public function execute($locator = null)
    {
        ...
        if (null === $locator) {
            throw new \InvalidArgumentException(
                'No entity manager passed to Spot Populator.'
            );
        }
        ...
        foreach ($this->quantities as $entityName => $number) {
            for ($i = 0; $i < $number; ++$i) {
                $insertedEntities[$entityName][] = $this->entities[
                    $entityName
