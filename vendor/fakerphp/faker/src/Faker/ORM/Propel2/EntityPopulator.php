...
class EntityPopulator
{
    ...
    {
        $this->columnFormatters = array_merge(
    ...
    public function guessColumnFormatters(\Faker\Generator $generator)
    {
        ...
        $columnTypeGuesser = new \Faker\ORM\Propel2\ColumnTypeGuesser(
            $generator
        );
        ...
        foreach ($tableMap->getColumns() as $columnMap) {
            ...
            if ($columnMap->isForeignKey()) {
                $relatedClass = $columnMap
                    ...
                    ->getForeignTable()
                    ->getClassname();
                $formatters[$columnMap->getPhpName()] = static function (
                    $inserted
                ) use ($relatedClass, $generator) {
                    $relatedClass = trim($relatedClass, '\\');

                    return isset($inserted[$relatedClass])
                        ? $generator->randomElement($inserted[$relatedClass])
                        : null;
                };
    ...
    public function guessModifiers(\Faker\Generator $generator)
    {
        ...
        foreach ($tableMap->getBehaviors() as $name => $params) {
            switch ($name) {
                case 'nested_set':
                    $modifiers['nested_set'] = static function (
                        $obj,
                        $inserted
                    ) use ($class, $generator) {
                        if (isset($inserted[$class])) {
                            ...
                            $parent = $queryClass
                                ::create()
                                ->findPk(
                                    $generator->randomElement($inserted[$class])
                                );
                ...
                case 'sortable':
                    $modifiers['sortable'] = static function (
                        $obj,
                        $inserted
                    ) use ($class, $generator) {
                        $obj->insertAtRank(
