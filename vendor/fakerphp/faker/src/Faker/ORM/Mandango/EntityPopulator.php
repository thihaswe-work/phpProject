...
class EntityPopulator
{
    ...
    public function guessColumnFormatters(
        ...
        $columnTypeGuesser = new \Faker\ORM\Mandango\ColumnTypeGuesser(
            $generator
        );
        ...
        // references
        foreach (
            array_merge($metadata['referencesOne'], $metadata['referencesMany'])
            as $referenceName => $reference
        ) {
            ...
            $formatters[$referenceName] = static function (
                $insertedEntities
            ) use ($referenceClass) {
                if (isset($insertedEntities[$referenceClass])) {
                    return Base::randomElement(
                        $insertedEntities[$referenceClass]
                    );
                }
