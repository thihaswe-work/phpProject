...
class EntityPopulator
{
    ...
    public function guessColumnFormatters($populator)
    {
        ...
        $guessers = $populator->getGuessers() + [
            'ColumnTypeGuesser' => new ColumnTypeGuesser(
                $populator->getGenerator()
            ),
        ];
    ...
    public function guessModifiers()
    {
        ...
        foreach ($belongsTo as $assoc) {
            $modifiers['belongsTo' . $assoc->name()] = function (
                $data,
                $insertedEntities
            ) use ($assoc) {
                $table = $assoc->target();
                ...
                if (empty($foreignKeys)) {
                    throw new \Exception(
                        sprintf(
                            ...
                            $this->getTable($this->class)->table(),
                            $assoc->table()
                        )
                    );
                }
    ...
    public function execute($class, $insertedEntities, $options = [])
    {
        ...
        foreach ($this->columnFormatters as $column => $format) {
            if (null !== $format) {
                $entity->{$column} = is_callable($format)
                    ? $format($insertedEntities, $table)
                    : $format;
            }
        }
